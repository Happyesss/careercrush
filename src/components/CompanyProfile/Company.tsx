    ...
    <div className="w-3/4">
      <div className="relative">
        <img
          ...
          src={((): string => { const mod = require(`../../assets/images/bgimg.jpg`); return typeof mod === 'string' ? mod : (mod?.default?.src ?? mod?.src ?? mod?.default ?? ''); })()}
          alt=""
        />
        <img
          className="w-36 h-36 rounded-3xl -bottom-1/4 absolute left-5 border-cape-cod-950 border-8 bg-cape-cod-950"
          src={((): string => { const mod = require(`../../assets/Icons/Google.png`); return typeof mod === 'string' ? mod : (mod?.default?.src ?? mod?.src ?? mod?.default ?? ''); })()}
          alt=""
        />
      </div>
