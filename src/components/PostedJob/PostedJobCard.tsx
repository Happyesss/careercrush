...
const PostedJobCard = (props: any) => {
  ...
  ${props.id === Number(id) ? "border-2 border-primary" : ""} 
      ...
      <div className=""> 
            ...
            <div className={`text-xs mb-1 flex text-[#434343df] ${isDarkMode ? "text-[#a8a8a8c9]" :"text-[#434343df]"} `}> <span className="flex items-center"><IconMapPin  className="h-4 font-light w-4 mr-1"/> {props.location}</span>  <span className="ml-3 flex items-center"> <IconUser className="h-4 font-light w-4 mr-1"/> {props.applicants?.length == null ? "0" : props.applicants?.length}</span></div>
